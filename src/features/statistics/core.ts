...
export function getStatisticsWindow(
  ...
  return {
    comparisonLabel:
      rangeKey === "30d" ? "vs previous 30 days" : "vs previous 12 weeks",
    end: formatDateKey(today),
...
function getExerciseSelectionKey(
  record: Pick<StatisticsRow, "exerciseId" | "exerciseNameSnapshot">,
) {
  return (
    record.exerciseId ??
    `snapshot:${record.exerciseNameSnapshot.trim().toLowerCase()}`
  );
}
...
export function buildExerciseOptions(currentSessions: StatisticsSession[]) {
  ...
  for (const session of currentSessions) {
    ...
    for (const record of session.records) {
      ...
      if (
        !seenKeys.has(selectionKey) &&
        !existing.sessionIds.has(session.sessionId)
      ) {
  ...
  const matchingSessions = currentSessions
    .map((session) => {
      ...
      const bestSet = matchingRecords.reduce((currentBest, candidate) => {
        ...
        if (
          candidate.weight === currentBest.weight &&
...
export function getStatisticsPageDataFromSessions(
